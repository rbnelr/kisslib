...
namespace kiss {
	...
	inline int indexof (std::vector<VT>& vec, T& r, EQUAL are_equal) {
	...
	inline int indexof (std::vector<VT>& vec, T& r) {
		for (int i=0; i<(int)vec.size(); ++i)
			if (vec[i] == r)
				return i;
		return -1;
	...
	// bool are_equal(VT const& l, T const& r)
	template <typename VT, typename T, typename EQUAL>
	inline bool contains (std::vector<VT>& vec, T& r, EQUAL are_equal) {
		for (auto& i : vec)
			if (are_equal(i, r))
				return true;
		return false;
	...
	template <typename VT, typename T>
	inline bool contains (std::vector<VT>& vec, T& r) {
		for (auto& i : vec)
			if (i == r)
				return true;
		return false;
